...
export function SummaryCard({ values }: SummaryCardProps) {
  ...
  return (
    <aside className="sticky top-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900/70">
      <button
        type="button"
      ...
      <div className={`${isOpen ? "block" : "hidden"} mt-3 space-y-3 md:mt-0 md:block`}>
        <h3 className="hidden text-sm font-semibold text-slate-900 dark:text-slate-100 md:block">Resumen</h3>
        <dl className="space-y-2 text-sm">
          <div>
            <dt className="text-slate-500">Empresa</dt>
            <dd className="font-medium text-slate-800 dark:text-slate-200">{companyLabels[values.companyType]}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Sector</dt>
            <dd className="font-medium text-slate-800 dark:text-slate-200">{sectorLabels[values.sector]}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Servicio</dt>
            <dd className="font-medium text-slate-800 dark:text-slate-200">{serviceLabels[values.service]}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Entregables</dt>
            <dd className="font-medium text-slate-800 dark:text-slate-200">{deliverables.join(", ") || "Sin seleccionar"}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Piezas</dt>
            <dd className="font-medium text-slate-800 dark:text-slate-200">{values.piecesCount}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Urgencia</dt>
            <dd className="font-medium text-slate-800 dark:text-slate-200">{urgencyLabels[values.urgency]}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Extras</dt>
            <dd className="font-medium text-slate-800 dark:text-slate-200">{extras.join(", ") || "Sin extras"}</dd>
          </div>
        </dl>
      </div>
    </aside>
